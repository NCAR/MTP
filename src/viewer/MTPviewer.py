...
class MTPviewer(QMainWindow):
    ...
    def writeEng2(self):
        ...
        for var in self.client.reader.getVarList('M01line'):
            ...
            if self.client.reader.getVar('M01line', var) == '':
                val = "%10s" % ' '
                volts = ' '
            else:
                val = "%04d" % int(self.client.reader.getVar('M01line', var))
                volts = "%+06.2fV" % self.client.reader.getCalcVal('M01line',
                                                                   var,
                                                                   'volts')
    ...
    def writeEng3(self):
        ...
        for var in self.client.reader.getVarList('M02line'):
            ...
            # Check for missing values in the MTP UDP feed, indicated by ,,
            # and read in as the string ''
            if self.client.reader.getVar('M02line', var) == '':
                val = "%10s" % ' '
            else:
                val = "%04d" % int(self.client.reader.getVar('M02line', var))
            value = "--"  # Until code value calc, put out -- as a placeholder
            # value = "%06.2fV" % self.client.reader.getVoltage('M02line', var)

            self.eng3.appendPlainText(name + "\t" + val + "  " + value)
