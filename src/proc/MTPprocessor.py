...
class MTPprocessor(QMainWindow):
    ...
    def __init__(self, viewer, client, mthp, tb, parent=None):
        """
        ...
        self.tb = tb  # True if writing TB data to an ascii file
    ...
    def setFile(self):
        ...
        if self.tb:
            selectedTbFile = selectedRawFile.replace("Raw", "TB")
            self.client.reader.openTB(selectedTbFile + "_tb.txt")
        ...
        while haveData:
            ...
            if haveData is True:  # Found a complete raw scan
                if not self.MTHP:
                    ...
                    # Save the brightness temperatures to an ascii file: time,
                    # lat, lon, alt, angle, values
                    if self.tb:
                        self.client.reader.writeTB(self.client.reader,
                                                   self.client.configfile)
