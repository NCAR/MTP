...
0.00, -12.00, -25.00, -42.00, -80.00''')
    ...
    def initSaveIWGFile(self, flightNumber, projectName):
        ...
        if not path.exists:
            makedirs(IWG1Path)
        saveIWGFileName = IWG1Path + '\\' + "IWG_" + time.strftime("%Y%m%d") +\
    ...
    def initSaveDataFile(self, flightNumber, projectName):
        # Make data file path
        path = os.path.dirname('C:\\Users\\lroot\\Desktop\\' + projectName +
                               '\\data\\')
        if not os.path.exists:
            os.makedirs(path)
        saveDataFileName = path+time.strftime("%Y%m%d") + '_' + \
            time.strftime("%H%M%S") + '_' + flightNumber + '.mtp'
        for filename in glob.glob(path + '\\*_' + flightNumber + '.mtp'):
            logging.debug("File exists")
            saveDataFileName = filename
